...
if project_num == 1:
    ...
    if step == 'Business Understanding':
        ...
        st.write('''* Công ty X chủ yếu phát hành và bán sản phẩm là CD tới khách hàng.
...
* Dữ liệu là lịch sử mua hàng đến cuối tháng 6 năm 1998 của 23,570 khách hàng cá nhân đã mua hàng lần đầu tại CDNOWS kể từ Quý 1 năm 1997.

* Dữ liệu nằm tổng cộng có 69,659 dòng dữ liệu, bao gồm:
...
### Thực hiện phân cụm khách hàng để chăm sóc khách hàng tốt hơn ''')
        
        image2 = st.image('customer-segment.png')

        st.write('''* Phân khúc/nhóm/cụm khách hàng (market segmentation – còn được gọi là phân khúc thị trường) là quá trình nhóm các khách hàng lại với nhau dựa trên các đặc điểm chung. Nó phân chia và nhóm khách hàng thành các nhóm nhỏ theo đặc điểm địa lý, nhân khẩu học, tâm lý học, hành vi (geographic, demographic, psychographic, behavioral) và các đặc điểm khác.
...
#### Mục tiêu của phân khúc khách hàng

''')
        
        image3 = st.image('How-to-conduct-a-customer-segmentation-study-Customer-Survey.png')

        st.write('''* Để xây dựng các chiến dịch tiếp thị tốt hơn
...
### Xây dựng giải pháp phân cụm khách hàng theo RFM.''')
        ...
        image4 = st.image('RFM_model.png')

        st.write('''* Phân tích RFM (Recency, Frequency, Monetary) là phương pháp tiếp cận dựa trên hành vi của khách hàng để nhóm thành các phân khúc. RFM phân nhóm khách hàng trên cơ sở các giao dịch mua hàng trước đó của họ, nhằm mục đích phục vụkhách hàng tốt hơn. 
* RFM giúp người quản lý xác định được khách hàng tiềm năng để kinh doanh có lợi nhuận hơn. Ngoài ra, nó giúp các nhà quản lý thực hiện các chiến dịch quảng cáo hiệu quả cho dịch vụ được cá nhân hóa.
...
### Đưa ra chiến dịch quảng cáo, bán hàng, chăm sóc khách hàng phù hợp cho mỗi nhóm. ''')
        ...
        * Doanh thu từ việc bán giảm mạnh trong khoảng từ tháng 01/1997 đến tháng 04/1997, sau đó không có sự thay đổi quá lớn ''')
        ...
        st.sidebar.divider()                           
        RFM_Analysis_ON = st.sidebar.toggle('RFM Analysis')
        RFM_Kmeans_ON = st.sidebar.toggle('RFM + K-means')
        RFM_HierarchicalClustering_ON = st.sidebar.toggle('RFM + Hierarchical Clustering')
        RFM_Kmeans_BigData_ON = st.sidebar.toggle('RFM + K-means (Big Data)')
        Compare_models_ON = st.sidebar.toggle('Compare models')
        st.sidebar.divider()
...
=> Có trục trặc lớn về sản phẩm hoặc cơ cấu quản lý vận hành của công ty này khá yếu. ''')
...
Trong phạm vi giới hạn thời gian nên tạm thời chúng ta tạm dừng phân tích sâu đến đây. ''')
        ...
        **[Tài liệu tham khảo](https://blog.tomorrowmarketers.org/phan-tich-rfm-la-gi/)**''')
        ...
        if RFM_Kmeans_ON:
            ...
            st.markdown('### Áp dụng mô hình K-Means:')
            ...
            # kmean_models = {}
            # for ret in results:
            #     kmeans_distortions._append(ret[0])
...
* Phần lớn số lượng khách hàng nằm ở nhóm 0 (73.31%) và thấp nhất là ở nhóm 3 (0.01%)''')
        ...
        if RFM_HierarchicalClustering_ON:
            ...
            st.markdown('### Áp dụng mô hình Hierarchical Clustering:')
...
* Phần lớn số lượng khách hàng có tỷ lệ 80.93% và thấp nhất có tỷ lệ 0.01% ''')     
        ...
        if RFM_Kmeans_BigData_ON:
            ...
            st.markdown('### Áp dụng mô hình K-Means của Big Data:')
            ...
            #     silhouette_list._append(silhouette)
...
* Cluster 1 - **Champions**: tỷ lệ thấp nhất (0.01%) RecencyMean = 2 FrequencyMean = 189 MonetaryMean = 9980, đây là những khách hàng có thời gian mua gần nhất, mua sắm thường xuyên nhất và chi tiêu nhiều nhất. Tăng cường các chương trình giảm giá cho nhóm này, giới thiệu ngay lập tức cho họ khi có sản phẩm mới. ''')
        ...
        st.header('Thực hiện phân khúc cho dữ liệu mới')
        ...
        if type=="Upload":
            ...
            if uploaded_file_1 is not None:
                data_df = pd.read_csv(uploaded_file_1, names=['customer_id', 'date', 'quantity', 'price'], sep = "\s+", index_col=None)
        ...
        if type=="Input":     
            ...
            if 'data' not in st.session_state:
                st.session_state.data = pd.DataFrame(columns=['customer_id', 'date', 'quantity', 'price'])

            # Create Streamlit app
            st.markdown("#### Nhập các dữ liệu sau")
            st.write('Sau khi nhập đủ 4 số liệu của 1 khách hàng thì bấm **Add Row** và tiếp tục nhập dòng kế tiếp cho đến khi đủ dữ liệu')

            # Add input fields for user to enter data
            customer_id = st.number_input("Nhập mã khách hàng:", value=0)
            date = st.number_input("Nhập ngày mua hàng theo dạng năm-tháng-ngày (VD:19970112):",value=0)
            quantity = st.number_input("Nhập số lượng hàng mua:", value=0)
            price = st.number_input("Nhập số tiền đã chi:", value=0)

            # Create a button to add the data as a new row
            if st.button("Add Row"):
                if customer_id and date and quantity and price > 0:
                    # Append the entered data to the DataFrame
                    
                    st.session_state.data = st.session_state.data._append({'customer_id':customer_id, 'date':date, 
                                                                        'quantity':quantity, 'price':price}, ignore_index=True)
        ...
        if flag:
            ...
            if len(data_df)>0:
                ...
                df_RFM_pred = data_df.groupby('customer_id').agg({'date': Recency,
                                                        'customer_id': Frequency,
                                                        'price': Monetary })
                df_RFM_pred.columns = ['Recency', 'Frequency', 'Monetary']
                
                pre_df = scaler.transform(df_RFM_pred)

                pre_df['id'] = pre_df.index

                spark_df = spark.createDataFrame(pre_df)
                ...
                df_pred_spark = predictions.toPandas()
                df_pred_spark = df_pred_spark.set_index('id')
                
                segment_dict = {0:'Lost', 1:'Champions', 2:'Promising', 3:'About to Sleep', 4:'Hibernating', 5:'Loyal Customers'}
                df_pred_spark['Group_name'] = df_pred_spark['prediction'].map(segment_dict)

                df_RFM_pred['Spark_Cluster'] = df_pred_spark['Group_name']

                st.write('#### Kết quả:')
                st.dataframe(df_RFM_pred)

                                
                # Create a button to convert the DataFrame to a CSV file
                if st.button("Download results"):
                    df_RFM_pred.to_csv("Clustering_Results.csv", index=False)
                    st.success("Results downloaded successfully!")

                cluster = pd.read_excel('cluster.xlsx')
                st.write('**Mô tả chi tiết các nhóm**')
